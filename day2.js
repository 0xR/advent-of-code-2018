...
// part 1
const checked = input.toString().split('\n').map(hasTimes);
...
function getSame(a, b) {
  ...
  return a.split('').map((aChar, i) => aChar === bSplit[i] && aChar).filter(Boolean).join('')
}

function numberArray(start, finish) {
  return Array.from({ length: finish - start }).fill(0).map((_, i) => i + start);
}

function sortedComparison(inputStrings) {
  return([].concat(...inputStrings.map((input, index) => {
    return numberArray(index + 1, inputStrings.length).map(index2 => {
      return [input, inputStrings[index2], getSame(input, inputStrings[index2])]
    });
